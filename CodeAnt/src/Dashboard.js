...
const repositories = [
  ...
  { name: 'blog-website', language: 'HTML/CSS', size: '1876 KB', visibility: 'Public', updatedAt: '4 days ago' },
  { name: 'social-network', language: 'PHP', size: '5432 KB', visibility: 'Private', updatedAt: '7 days ago' }
];
  ...
  <div className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-4 text-sm text-gray-600 flex-wrap">
        <RepoDetails language={repo.language} size={repo.size} updatedAt={repo.updatedAt} />
      </div>
    </div>
  </div>
);
...
const VisibilityBadge = ({ visibility }) => (
  <span className={`px-2 py-1 text-xs rounded-full ${visibility === 'Public' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
    {visibility}
  </span>
);

const RepoDetails = ({ language, size, updatedAt }) => (
  <>
...
const NavItem = ({ icon: Icon, label, active }) => {
  ...
  const activeClasses = active ? "text-white bg-blue-600" : "text-gray-700 hover:bg-gray-100";
  return (
